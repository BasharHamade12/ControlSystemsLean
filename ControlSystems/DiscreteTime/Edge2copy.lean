...
-- ---------------------------------------------------------------
-- Helper lemma for Case B: separation in the direction space
-- ---------------------------------------------------------------
...
private lemma exists_proper_subface_of_boundary_point_caseA {n : ℕ} (P : Polytope n)
    (hp : SupportingHyperplane P) (hF_eq : F = ExposedFace hp)
    (δ_bound : CoeffVec n) (hδ_in_Ω : δ_bound ∈ P.Ω) (hδ_f_val : hp.f δ_bound = hp.c)
    (g_Ω : CoeffVec n →ₗ[ℝ] ℝ) (g_c : ℝ) (hg_Ω_support : ∀ x ∈ P.Ω, g_Ω x ≤ g_c)
  ...
  have hδ_in_ExF : δ_bound ∈ ExposedFace hp := hF_eq ▸ hδ_in_Ω
    -- but we need a proof of `hδ_bound_in_F` for that, we'll use `hδ_in_Ω` + `hδ_f_val`
    -- Actually we need `hδ_bound_in_F` to use `hF_eq ▸ ...`. We can reconstruct:
    have hδ_bound_in_F : δ_bound ∈ F := by
      rw [hF_eq]; exact ⟨hδ_in_Ω, hδ_f_val⟩
    ...
    have hδ_in_G : δ_bound ∈ G := by
      refine ⟨hδ_in_Ω, ?_⟩
      simp only [G, Pi.add_apply, LinearMap.add_apply]; linarith [hδ_f_val]
    have hG_sub_ExF : G ⊆ ExposedFace hp := by
      ...
      have h_fx_eq : hp.f x = hp.c := by
        simp only [Pi.add_apply, LinearMap.add_apply] at hx_sum; linarith
      exact ⟨hx_Ω, h_fx_eq⟩
    have hG_dim_lt : Module.finrank ℝ (affineSpan ℝ G).direction <
        Module.finrank ℝ (affineSpan ℝ F).direction := by
      have hG_dir_le_ker : (affineSpan ℝ G).direction ≤
      ...
      have hG_dir_le_F_dir : (affineSpan ℝ G).direction ≤
      ...
      have hv_not_dirG : v_dir ∉ (affineSpan ℝ G).direction :=
        fun h => hv_not_ker (hG_dir_le_ker h)
      have h_dir_ne : (affineSpan ℝ G).direction ≠ (affineSpan ℝ (ExposedFace hp)).direction :=
        fun h_eq => hv_not_dirG (h_eq ▸ hv_in_dir)
      have h_dir_strict : (affineSpan ℝ G).direction < (affineSpan ℝ (ExposedFace hp)).direction :=
...
private lemma exists_proper_subface_of_boundary_point_caseB {n : ℕ} (P : Polytope n)
    (hp : SupportingHyperplane P) (hF_eq : F = ExposedFace hp)
    (δ_bound : CoeffVec n) (hδ_bound_in_F : δ_bound ∈ F)
    ...
    (hF_dim : Module.finrank ℝ (affineSpan ℝ F).direction ≥ 2)
    (hF_compact : IsCompact F) (hF_convex : Convex ℝ F) (hF_sub_Ω : F ⊆ P.Ω)
    (h_int_nonempty : (interior P.Ω).Nonempty) (hΩ_convex : Convex ℝ P.Ω) (hΩ_closed : IsClosed P.Ω) :
    ∃ (G : Set (CoeffVec n)), IsExposedFace P G ∧ δ_bound ∈ G ∧
    Module.finrank ℝ (affineSpan ℝ G).direction < Module.finrank ℝ (affineSpan ℝ F).direction := by
  ...
  let C : Set V_dir := τ ⁻¹' ((Subtype.val : affF → CoeffVec n) ⁻¹' (intrinsicInterior ℝ F))
  ...
  have hC_eq_interior_A : C = interior A := by
    calc
      C = τ ⁻¹' ((Subtype.val : affF → CoeffVec n) ⁻¹' (intrinsicInterior ℝ F)) := by
        ext x; simp [C, τ]
      _ = τ ⁻¹' (interior ((Subtype.val : affF → CoeffVec n) ⁻¹' F)) := by
        rw [h_int_eq]
      _ = interior (τ ⁻¹' ((Subtype.val : affF → CoeffVec n) ⁻¹' F)) := by
  ...
  have h0_notin_C : (0 : V_dir) ∉ C := by
    intro h
    have hτ0 : (τ 0 : CoeffVec n) = δ_bound := by simp [τ, vadd_eq_add]
  ...
  have h_on_intF : ∀ y ∈ intrinsicInterior ℝ F, w_base y < c_w := by
    ...
    calc
      ...
      _ = w_base (v : CoeffVec n) + c_w := by dsimp [v]
  ...
  have h_closure_intF : closure (intrinsicInterior ℝ F) = F := by
    ...
    · intro x hx
      ...
      haveI : Filter.NeBot (nhdsWithin (0 : ℝ) (Set.Ioi 0)) :=
        nhdsWithin_neBot (Set.mem_Ioi.mpr (by norm_num : (0 : ℝ) < 1))
      exact mem_closure_of_tendsto h_tendsto h_event

  have hw_nonpos_F : ∀ x ∈ F, w_base x ≤ c_w := by
    intro x hx
    have hx_closure : x ∈ closure (intrinsicInterior ℝ F) := by rw [h_closure_intF]; exact hx
  ...
  · -- Case B1: w_base ≤ c_w on all vertices → λ = 1 works
    ...
    have h_support : ∀ x ∈ P.Ω, f_new x ≤ c_new := by
      ...
      have hv_w : w_base v ≤ c_w := by
        by_contra! hgt
        have : v ∈ S_verts := by
          dsimp [S_verts]; exact Finset.mem_filter.mpr ⟨hv, hgt⟩
        rw [hS_empty] at this; simp at this
      dsimp [f_new, c_new]
      nlinarith [hp.upper_bound v ((subset_convexHull ℝ _) hv), hlam_pos]
    have h_touches : ∃ x ∈ P.Ω, f_new x = c_new := ⟨δ_bound, hδ_in_Ω, by simp [f_new, hδ_f_val, c_new, c_w]⟩
    have h_nonzero : f_new ≠ 0 := by
      ...
      have hw_eq : w_base y = c_w := (mul_left_inj' (ne_of_gt hlam_pos)).mp h_eq
    ...
    have hδ_in_G : δ_bound ∈ G := ⟨hδ_in_Ω, by simp [f_new, hδ_f_val, c_new, c_w]⟩
    have hG_sub_F : G ⊆ F := by
      ...
      have h_v_le : ∀ v ∈ P.vertices, f_new v ≤ c_new := by
        ...
        dsimp [f_new, c_new]
        have hv_w : w_base v ≤ c_w := by
          have hvS_not : v ∉ S_verts := by simp [hS_empty]
          simp [S_verts, Finset.mem_filter, not_and] at hvS_not
          push_neg at hvS_not; exact not_lt.mp (hvS_not v hv)
        nlinarith [hp.upper_bound v ((subset_convexHull ℝ _) hv), hlam_pos]
      have h_exists_v_not_F : ∃ v ∈ P.vertices, w_poly v > 0 ∧ v ∉ F := by
        ...
        have h_x_in_F : x ∈ F := by
          ...
          have h_mem : x ∈ convexHull ℝ (P.vertices.filter (fun v => v ∈ F) : Set (CoeffVec n)) := by
            ...
            · intro y hy; exact hw_nonneg y (Finset.mem_of_mem_filter hy)
            · calc
    ...
    have h_dim_lt : Module.finrank ℝ (affineSpan ℝ G).direction <
      ...
      have hw_V_nonzero : w_V ≠ 0 := by
        ...
        have h_w_diff_lt : w_base (y - δ_bound) < 0 := by
          rw [h_w_y] at h_w_y_lt; linarith
        have h_zero : w_V ⟨y - δ_bound, hv⟩ = 0 := by
          simp [hzero]
        have : w_base (y - δ_bound) = 0 := h_zero
        linarith
      have h_dim_ker : Module.finrank ℝ (LinearMap.ker w_V) = Module.finrank ℝ V_dirF - 1 :=
        finrank_ker_eq_finrank_sub_one w_V hw_V_nonzero
      have h_iso : Module.finrank ℝ (LinearMap.ker w_V) = Module.finrank ℝ (V_dirF ⊓ LinearMap.ker w_base) := by
        let φ : LinearMap.ker w_V ≃ₗ[ℝ] ↥(V_dirF ⊓ LinearMap.ker w_base) := {
          toFun := fun x => ⟨x.1.1, ⟨x.1.2, by
            change w_base x.1.1 = 0; simpa [w_V, LinearMap.comp_apply, Submodule.subtype_apply] using x.2⟩⟩
          invFun := fun y => ⟨⟨y.1, y.2.1⟩, by
            change w_base y.1 = 0; exact y.2.2⟩
    ...
    have h_support : ∀ x ∈ P.Ω, f_new x ≤ c_new := by
      ...
      have hv_new : f_new v ≤ c_new := by
        ...
        · push_neg at hw_gt
          dsimp [f_new, c_new]; nlinarith
      linarith
    have h_touches : ∃ x ∈ P.Ω, f_new x = c_new :=
      ⟨δ_bound, hδ_in_Ω, by simp [f_new, hδ_f_val, c_new, c_w]⟩
    have h_nonzero : f_new ≠ 0 := by
      intro hzero
      rcases h_nonconst with ⟨y, hyF, hyw⟩
      have hy_f : hp.f y = hp.c := (hF_eq ▸ hyF).2
      have h_y : f_new y = 0 := by simpa [f_new, hy_f] using congrArg (fun f => f y) hzero
      have h_δ : f_new δ_bound = 0 := by simpa [f_new, hδ_f_val] using congrArg (fun f => f δ_bound) hzero
      dsimp [f_new, c_new] at h_y h_δ
      have h_eq_lam : lam * w_base y = lam * c_w := by nlinarith
      have h_wy_eq_cw : w_base y = c_w := (mul_left_inj' (ne_of_gt hlam_pos)).mp h_eq_lam
...
-- ---------------------------------------------------------------
-- The main lemma (now just a dispatcher)
-- ---------------------------------------------------------------
...
private lemma exists_proper_subface_of_boundary_point {n : ℕ} (P : Polytope n)
  ...
  have hF_relint_nonempty : (intrinsicInterior ℝ F).Nonempty :=
    Set.Nonempty.intrinsicInterior (isExposedFace_convex P ⟨hp, hF_eq⟩) ⟨δ_bound, hδ_bound_in_F⟩
  ...
  have hg_Ω_support : ∀ x ∈ P.Ω, g_Ω x ≤ g_c := by
    intro x hx
    have h_closed_le : IsClosed {y : CoeffVec n | g_Ω y ≤ g_c} :=
      isClosed_Iic.preimage (LinearMap.continuous_of_finiteDimensional g_Ω)
    have h_int_sub : interior P.Ω ⊆ {y | g_Ω y ≤ g_c} := fun y hy => le_of_lt (hf_Ω_strict y hy)
    have h_closure_Ω : closure (interior P.Ω) = P.Ω :=
      calc closure (interior P.Ω) = closure P.Ω :=
  ...
  · -- Case A
    exact exists_proper_subface_of_boundary_point_caseA P hp hF_eq δ_bound hδ_in_Ω hδ_f_val g_Ω g_c hg_Ω_support hg_Ω_nonconst hF_dim
  · -- Case B
    ...
    exact exists_proper_subface_of_boundary_point_caseB P hp hF_eq δ_bound hδ_bound_in_F hδ_bound_not_relint hδ_in_Ω hδ_f_val g_Ω g_c hg_Ω_support hg_Ω_const hF_dim hF_compact hF_convex hF_sub_Ω h_int_nonempty hΩ_convex hΩ_closed

-- ... (rest of the file remains unchanged, i.e. isExposedEdge_of_dim_1, exists_boundary_point_in_face_rootspace, descend_to_exposed_edge, polytope_direction_dim_pos, polytope_dim1_is_exposed_edge, exists_exposed_edge_through_vertex, lemma61_real, lemma61_complex, lemma61) ...
